...
export const servicesRelations = relations(services, ({ one }) => ({
  provider: one(providerProfiles, { fields: [services.providerId], references: [providerProfiles.id] }),
}));

export const portfolioRelations = relations(portfolio, ({ one }) => ({
  provider: one(providerProfiles, { fields: [portfolio.providerId], references: [providerProfiles.id] }),
}));

export const providerAvailabilityRelations = relations(providerAvailability, ({ one }) => ({
  provider: one(providerProfiles, { fields: [providerAvailability.providerId], references: [providerProfiles.id] }),
}));

export const providerDaysOffRelations = relations(providerDaysOff, ({ one }) => ({
  provider: one(providerProfiles, { fields: [providerDaysOff.providerId], references: [providerProfiles.id] }),
}));

export const bookingsRelations = relations(bookings, ({ one, many }) => ({
  customer: one(customerProfiles, { fields: [bookings.customerId], references: [customerProfiles.id] }),
  provider: one(providerProfiles, { fields: [bookings.providerId], references: [providerProfiles.id] }),
  service: one(services, { fields: [bookings.serviceId], references: [services.id] }),
  reviews: many(reviews),
  conversation: one(conversations, { fields: [bookings.id], references: [conversations.bookingId] }),
...
export const sliikOffersRelations = relations(sliikOffers, ({ one, many }) => ({
  customer: one(customerProfiles, { fields: [sliikOffers.customerId], references: [customerProfiles.id] }),
  responses: many(sliikOfferResponses),
}));

export const sliikOfferResponsesRelations = relations(sliikOfferResponses, ({ one }) => ({
  offer: one(sliikOffers, { fields: [sliikOfferResponses.offerId], references: [sliikOffers.id] }),
  provider: one(providerProfiles, { fields: [sliikOfferResponses.providerId], references: [providerProfiles.id] }),
}));

export const sliikDealsRelations = relations(sliikDeals, ({ one }) => ({
  provider: one(providerProfiles, { fields: [sliikDeals.providerId], references: [providerProfiles.id] }),
  service: one(services, { fields: [sliikDeals.serviceId], references: [services.id] }),
}));
